...
const longevityPercentInput = document.getElementById('longevity-percent');
...
if (bonusPercentInput) {
    bonusPercentInput.addEventListener('input', (e) => {
        ...
        if (gross > 0) {
            ...
            const longevity = gross * (longevityPercent / 100);
            const totalGross = gross + bonus + longevity;
            const results = calculateDeductions(totalGross);
            
            bonusInput.value = bonus > 0 ? formatWithCommas(bonus) : '';
            longevityInput.value = longevity > 0 ? formatWithCommas(longevity) : '';
...
if (longevityPercentInput) {
    longevityPercentInput.addEventListener('input', (e) => {
        longevityPercent = parseFloat(e.target.value) || 0;
        if (isUpdating) return;
        ...
        if (gross > 0) {
            ...
            const bonus = gross * (bonusPercent / 100);
            const longevity = gross * (longevityPercent / 100);
            const totalGross = gross + bonus + longevity + mealMoney + transportMoney;
            const results = calculateDeductions(totalGross);
...
if (daysInput) {
    daysInput.addEventListener('input', (e) => {
        workingDays = parseFloat(e.target.value) || 0;
        if (isUpdating) return;
        
        isUpdating = true;
        mealMoney = workingDays * 9000;
        transportMoney = workingDays * 1000;
        ...
        mealInput.value = mealMoney > 0 ? formatWithCommas(mealMoney) : '';
        transportInput.value = transportMoney > 0 ? formatWithCommas(transportMoney) : '';
        netInput.value = results.net > 0 ? formatWithCommas(results.net) : '';
        updateDisplay(results);
        isUpdating = false;
    });
...
if (mealInput) {
    mealInput.addEventListener('input', (e) => {
        if (isUpdating) return;
        handleInputFormatting(e.target, (val) => {
            isUpdating = true;
            mealMoney = val;
            const gross = parseRaw(grossInput.value);
            const bonus = gross * (bonusPercent / 100);
...
longevityInput.addEventListener('input', (e) => {
    if (isUpdating) return;
    handleInputFormatting(e.target, (longevity) => {
        isUpdating = true;
        
        let gross = 0;
        if (longevityPercent > 0) {
            gross = longevity / (longevityPercent / 100);
        } else {
            gross = parseRaw(grossInput.value);
        }
...
netInput.addEventListener('input', (e) => {
    ...
    handleInputFormatting(e.target, (net) => {
        ...
        const dailyAllowance = mealMoney + transportMoney;
        const gross = (totalGross - dailyAllowance) / (1 + (bonusPercent + longevityPercent) / 100);
        const bonus = gross * (bonusPercent / 100);
        const longevity = gross * (longevityPercent / 100);
...
const inputs = [grossInput, bonusInput, longevityInput, mealInput, transportInput, netInput];
